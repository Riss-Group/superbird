...
class InspectionVehicleDetails(models.TransientModel):
    """Inspection Vehicle Details"""
    _name = 'inspection.vehicle.details'
    ...
    def default_get(self, field):
        res = super(InspectionVehicleDetails, self).default_get(field)
        res['inspection_job_card_id'] = self._context.get('active_id')
    ...
    def get_fleet_vehicle_details(self):
        for rec in self:
            if rec.fleet_vehicle_id and rec.vehicle_type == 'fleet_vehicle':
    ...
    def get_register_vehicle_details(self):
        for rec in self:
            if rec.register_vehicle_id and rec.vehicle_type == 'customer_vehicle':
    ...
    def action_add_vehicle_details(self):
        ...
        inspection_job_card_id = self.env['inspection.job.card'].browse(rec)
        data = {
        ...
        inspection_job_card_id.write(data)
