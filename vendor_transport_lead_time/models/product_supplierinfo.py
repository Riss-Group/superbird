...
class ProductSupplierinfo(models.Model):
    ...
    delay = fields.Integer(compute="_compute_delay", inverse="_inverse_delay", store=True, required=False)
    supplier_delay = fields.Integer(string="Supplier Lead Time", default=0, required=True, help="Supplier lead time in days.")
    transport_delay = fields.Integer(string="Transport Lead Time", default=0, required=True, help="Transport lead time in days.")
    ...
    def _check_supplier_delay(self):
        for seller in self:
            if seller.supplier_delay < 0:
                raise ValidationError(self.env._("You can't set a delay inferior to the transport delay."))
