...
class StockMove(models.Model):
    ...
    def _action_assign(self, force_qty=False):
        for move in self :
            if move.picking_code == 'internal' and move.picking_type_id.is_pick:
                incoming_waiting_picking = self.search([('product_id','=', move.product_id.id),('state','=','confirmed')])
                incoming_waiting_picking._check_waiting_sale_picking()
        res = super(StockMove, self)._action_assign(force_qty)
        for move in self:
            if move.picking_code == 'internal' and move.picking_type_id.is_put_away:
                self.filtered(lambda m: m.picking_code == 'internal')._check_waiting_sale_picking()
        return res

    def _check_waiting_sale_picking(self):
        for move in self:
            outgoing_waiting_moves = self.search([('state','in', ['confirmed','partially_available']), ('picking_code','=', 'internal'), ('picking_type_id.is_pick','=', True),
    ...
    def update_move_destination_location(self, crossdock_location=False, sale_qty=0):
        if crossdock_location and sale_qty and crossdock_location not in self.picking_id.move_ids.filtered(lambda m: m.product_id == self.product_id).mapped('location_dest_id'):
