...
def sale_order_create(payload: DataPayloadSO, env: Environment = Depends(odoo_env)) -> ReturnValuesSO:
    ...
    try:
        ...
        for line in payload.orderdetail:
            ...
            if not product_id:
                return_values.message = f"Could not find a matching Odoo product that matches the sku provided of [{line.itemno}]. Perhaps the replacement_id is an issue?"
                return return_values   
        ...
        return_values.message = f"EX Occured: {e}"
        return return_values
    output = ReturnValuesSO()
    output.sale_order_id = str(order_id.id)
    output.sale_order_name = str(order_id.name)
