...
class ProductTemplate(models.Model):
    ...
    def write(self, vals):
        ...
        if 'options_package_ok' in vals or 'sale_ok' in vals or 'detailed_type' in vals:
            for record in self:
                record._onchange_options_sale()
        if 'package_service_template_id' in vals:
            for record in self:
                record._onchange_package_service_template_id()
        return res
