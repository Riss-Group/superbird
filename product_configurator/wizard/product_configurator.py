...
class ProductConfigurator(models.TransientModel):
    ...
    def apply_onchange_values(self, values, field_names, field_onchange):
        ...
        if not state:
            state = self.state
        # Retrieve product attribute values if provided
        value_ids = values.get("value_ids", [])
        if value_ids and isinstance(value_ids, list):
            # Ensure the structure matches and extract the correct attribute values
            cfg_vals = self.env["product.attribute.value"].browse(
                value_ids[0][2]
                if len(value_ids[0]) > 2 and isinstance(value_ids[0][2], list)
                else []
            )
        else:
            cfg_vals = self.value_ids

        field_type = type(field_names)
        field_prefix = self._prefixes.get("field_prefix")
        ...
        if field_type == list and (
            not local_field_name and not local_custom_field and not local_domain_prefix
        ):
    ...
    def _generate_dependency_attributes(
        ...
        for dependee_field, val_ids in attr_depends.items():
            ...
            if field_type != "many2many":
                readonly_str += f" or {dependee_field} not in {str(list(val_ids))}"
            if (
                attr_line.required
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if session.value_ids:
                if (
                    isinstance(wz_value_ids, list)
                    and wz_value_ids
                    and isinstance(wz_value_ids[0], list)
                    and len(wz_value_ids[0]) > 2
                ):
                    # Only update if wz_value_ids[0][2] is empty.
                    if not wz_value_ids[0][2]:
                        vals.update({"value_ids": [(6, 0, session.value_ids.ids)]})
                # If wz_value_ids is an empty list.
                elif not wz_value_ids:
                    vals.update({"value_ids": [(6, 0, session.value_ids.ids)]})
